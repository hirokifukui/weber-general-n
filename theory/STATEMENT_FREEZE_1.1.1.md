# STATEMENT_FREEZE_1.1.1 — frozen statements (2026-09-22, the metadata repair round; opened on the GPT public-state audit of the published v1.1.0)

Status: FROZEN. NO statement change and NO proof-text change in 1.1.1: the 38 single-source files proofs/statements/*.tex are byte-identical to the r21..v1.1.0 seals (md5 list digest 9c7b3f5775341416eb756c5dd4e6472f unchanged), and proofs/*.tex are byte-identical to v1.1.0. The 1.1.1 content, all metadata: Table 7 CI provenance refreshed to the v1.1.0 release run (35692588284, commit 9780ac69); TRUST.md header and CI paragraph brought up to the executed clean-checkout record (the pre-v1.0.0 'not yet executed' text was stale since the first release); README trust-model L label corrected to 'published or preprint' (the v1.0.2 correction had missed it) and a current-article pointer added; CITATION.cff preferred-citation title set to the MoC article title; .zenodo.json description MSC and asset list synchronized; CI runner pinned to ubuntu-24.04. The v1.1.0 file follows below.

---- v1.1.0 file (history) ----
# STATEMENT_FREEZE_1.1.0 — frozen statements (2026-09-22, the MoC recast round; opened after the JTNB decision of 2026-09-21)

Status: FROZEN. NO statement change in 1.1.0: the 38 single-source files proofs/statements/*.tex are byte-identical to the r21..v1.0.2 seals (md5 list digest 9c7b3f5775341416eb756c5dd4e6472f unchanged). The 1.1.0 content: the paper is recast for Mathematics of Computation as paper/draft/main_1.1.0.tex (computational-problem introduction; certificate soundness with the verifier specification as Algorithm 3.1; Algorithms 4.2/4.3 with ball-operation counts; resource table; reproducibility-and-trust section; AMS-compliant title/abstract/heading macros; MSC now 11Y40, 11R18 primary); the proof text of Cor.~T replaces the decimal quotients of the Example paragraph by the certified integer comparisons (ERRATA_1.1.0 E1.1.0-1; proofs/cor_T.tex; statement unchanged); no Lean file, certificate or evidence artifact is touched. The v1.0.2 file follows below.
